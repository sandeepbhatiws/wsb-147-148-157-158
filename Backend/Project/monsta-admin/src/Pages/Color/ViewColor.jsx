...
export default function ViewCategory() {
  ...
  const changeStatus = () => {
    if(checkedValues.length > 0){
      axios.put(import.meta.env.VITE_API_BASE_URL+import.meta.env.VITE_CHANGESTATUS_COLOR_URL, {
        id : checkedValues
      })
      .then((result) => {
        if(result.data._status == true){
  ...
  const deleteRecords = () => {
    if(checkedValues.length > 0){
      if(confirm('Are you sure you want to delete ?')){
        axios.put(import.meta.env.VITE_API_BASE_URL+import.meta.env.VITE_DELETE_COLOR_URL, {
        id : checkedValues
      })
      .then((result) => {
        if(result.data._status == true){
      ...
      <div className={`bg-gray-50 px-2 py-5 max-w-[1220px] duration-[1s] mx-auto mt-10 ${activeFilter ? "hidden" : "block"}`}>
        ...
        <form className="flex max-w-sm">
          <div className="relative w-full">
            <input
              type="text"
              id="simple-search" onKeyUp={ searching }
                    ...
                    <tr>
                      <th scope="col" class="p-4">
                        <div class="flex items-center">
                          ...
                          checked = { checkedValues.length == colors.length ? 'checked' : '' }
                        ...
                        colors.map((value, index) => {
                          ...
                          <tr key={index} class="bg-white  dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                            ...
                            <td class=" py-4">
                              ...
                              <Link to={`/color/update/${value._id}`} >
