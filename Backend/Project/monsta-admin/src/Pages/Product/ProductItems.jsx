...
export default function ViewProduct() {
  ...
  useEffect(() => {
    axios.post(import.meta.env.VITE_API_BASE_URL + import.meta.env.VITE_VIEW_PRODUCT_URL, {
      name: searchName,
      parent_categories_ids: parentCategory,
      page: currentPage,
    })
      .then((result) => {
        if (result.data._status == true) {
  ...
  const getAllValues = () => {
    if (checkedValues.length == products.length) {
      setCheckedValues([]);
    } else {
      var data = [];
      products.forEach((v) => {
  ...
  const changeStatus = () => {
    if (checkedValues.length > 0) {
      axios.put(import.meta.env.VITE_API_BASE_URL + import.meta.env.VITE_CHANGESTATUS_PRODUCT_URL, {
        id: checkedValues
      })
        .then((result) => {
          if (result.data._status == true) {
  ...
  const deleteRecords = () => {
    if (checkedValues.length > 0) {
      if (confirm('Are you sure you want to delete ?')) {
        axios.put(import.meta.env.VITE_API_BASE_URL + import.meta.env.VITE_DELETE_PRODUCT_URL, {
          id: checkedValues
        })
          .then((result) => {
            if (result.data._status == true) {
                      ...
                      <th scope="col" class="p-4">
                        <div class="flex items-center">
                          <input onClick={getAllValues}
                            checked={checkedValues.length == products.length ? 'checked' : ''}
                        ...
                        products.map((value, index) => {
                          return (
                            <tr key={index} class="bg-white  dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                              ...
                              <td class=" py-4">
                                {
                                  value.parent_categories_ids != '' && value.parent_categories_ids.length > 0
                                    ?
                                    value.parent_categories_ids.map((v, i) => {
                                      return (
                                        v.name+', '
                                      )
                                    })
